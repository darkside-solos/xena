...
async function replyAudio(bot, msg, buffer, isVoice = false) {
  if (isVoice) {
    await bot.sendVoice(msg.chat.id, buffer, { reply_to_message_id: msg.message_id });
  } else {
    await bot.sendAudio(msg.chat.id, buffer, { reply_to_message_id: msg.message_id });
  }
}
...
async function replySticker(bot, msg, buffer) {
  await bot.sendSticker(msg.chat.id, buffer, { reply_to_message_id: msg.message_id });
}
...
function startTelegramBot(config) {
  if (botInstance) {
    try { botInstance.stopPolling(); botInstance = null; } catch (_) {}
  }

  const token = config?.TELEGRAM_BOT_TOKEN;

  if (!token || token === 'YOUR_TELEGRAM_BOT_TOKEN') {
    log('⏳ No Telegram token — waiting for website deploy to activate bot');
    return null;
  }
  ...
  bot.getMe().then(me => {
    ...
    log(`❌ getMe failed — token may be invalid: ${e.message}`);
    try { bot.stopPolling(); botInstance = null; } catch (_) {}
  });
  ...
  bot.on('message', async (msg) => {
    ...
    try {
      ...
      console.error('FULL ERROR:', err); // ✅ shows exact crash in Render logs
  ...
  // ── Poll errors — no infinite loop on bad token ───────────────────
  bot.on('polling_error', (err) => {
    log(`TG polling error: ${err.message}`);
    if (err.message.includes('404') || err.message.includes('ETELEGRAM')) {
      log('❌ Invalid token — bot stopped. Re-deploy via website with a valid token.');
      try { bot.stopPolling(); botInstance = null; } catch (_) {}
      return;
    }
    setTimeout(() => {
