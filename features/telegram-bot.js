...
function startTelegramBot(config) {
  ...
  // ── BYPASS — don't start if no valid token ────────────────────────
  ...
  // ── Poll errors — FIXED: no infinite loop on bad token ───────────
  bot.on('polling_error', (err) => {
    log(`TG polling error: ${err.message}`);

    // 404 = invalid token — kill immediately, no retry loop
    ...
    // Network or other errors — safe to retry after 5s
    setTimeout(() => {
...
// ── Required by server.js and index.js ───────────────────────────────
